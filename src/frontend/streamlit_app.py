...
from utils.dropbox.dropbox_io import download_model_from_dropbox
...
class DataManager:
    ...
    def _load_match_data(self):
        ...
        try:
            path = os.path.abspath(os.path.join(current_dir, '../../data/prepared/Belgium_league_2526.csv'))
            df = pd.read_csv(path)
    ...
    def _load_model(_self):
        ...
        # return None
        model = download_model_from_dropbox(dropbox_path=settings.dropbox_model_path)
    ...
    def _prepare_features(self, home_team: str, away_team: str):
        ...
        try:
            df = load_csv(filedir="prepared", filename="Belgium_league_2526.csv")
...
def render_team_card(team_name: str, data_manager: DataManager, default_emoji: str):
    ...
    # Last 5 matches
    try:
        historical_data = load_csv(filedir="prepared", filename="Belgium_league_2526.csv")
        ...
        st.markdown(
            "<div style='text-align: center; font-size: 0.8rem; color: #ff6b6b; margin: 10px 0; font-style: italic;'>No recent matches available</div>",
